...
const API = 'https://cit3.internshipstudio.com/admin/react-api/api/support/non_qualified_links.php';
const POLL_MS = 5000;
const PROTECTED = '';   // no protected channel for non-qualified
...
const ActBtn = ({ label, onClick, grad, small }) => (
    <button onClick={onClick}
        style={{
            padding: small ? '4px 9px' : '4px 12px', border: 'none', borderRadius: 6,
            fontSize: 11.5, fontWeight: 700, cursor: 'pointer', color: '#fff', fontFamily: 'inherit',
...
export default function PlacementLinksForNonQualified() {
    ...
    return (
        <>
        <Helmet>
        <title>Non Qualified Whatsapp Link | Admin Panel</title>
      </Helmet>
            <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');
        .rcl-root * { box-sizing:border-box; font-family:'Plus Jakarta Sans',sans-serif; }
        .rcl-tr:hover td { filter:brightness(.97); }
        @keyframes rcl_spin { to { transform:rotate(360deg); } }
        .rcl-spin { display:inline-block;width:16px;height:16px;border:2px solid #ede9fe;border-top-color:#4f46e5;border-radius:50%;animation:rcl_spin .7s linear infinite; }
        @keyframes rcl_pulse { 0%,100%{opacity:1}50%{opacity:.5} }
        .live-dot { display:inline-block;width:7px;height:7px;border-radius:50%;background:#22c55e;animation:rcl_pulse 2s infinite; }
      `}</style>

            <div className="rcl-root" style={{
                ...
                <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
                    <div>
                        <div style={{ fontSize: 17, fontWeight: 800, color: '#1e293b' }}>🚫 Non Qualified Community Links</div>
                        <div style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>
                            Manage WhatsApp groups for non-qualified students — <code style={{ background: '#ede9fe', padding: '1px 5px', borderRadius: 4, color: '#4f46e5', fontSize: 11 }}>wa_cit_non_qualified_club</code>
                        </div>
                    </div>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        ...
                        <button onClick={handleAdd}
                            ...
                            + Add Community Link For Non Qualified
                        </button>
                    </div>
                </div>
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ fontSize: 11, color: '#94a3b8' }}>Active rows highlighted in green</div>
                    </div>
                            ...
                            <tbody>
                                {!rows.length && !loading ? (
                                    <tr><td colSpan={6} style={{ textAlign: 'center', padding: 50, color: '#94a3b8' }}>
                                        ...
                                        <div style={{ fontSize: 13, fontWeight: 600, color: '#64748b' }}>No non-qualified community links found</div>
                                    </td></tr>
                                ) : rows.map(row => {
                                    const isActive = row.status === 'active';
                                    const isEditing = editId === row.id;
                                    const isProtected = PROTECTED && row.community_link === PROTECTED;
                                    return (
                                        <tr key={row.id} className="rcl-tr">
                                            {/* Name */}
                                            <td style={tdS(isActive)}>
                                                {isEditing
                                                    ? <InlineInput value={editForm.community_name} onChange={v => setEditForm(p => ({ ...p, community_name: v }))} />
                                            ...
                                            <td style={{ ...tdS(isActive), maxWidth: 300 }}>
                                                {isEditing
                                                    ? <InlineInput value={editForm.community_link} onChange={v => setEditForm(p => ({ ...p, community_link: v }))} />
                                                    : (
                                            ...
                                            <td style={tdS(isActive)}>
                                                {isEditing
                                                    ? <InlineSelect value={editForm.status} onChange={v => setEditForm(p => ({ ...p, status: v }))} />
                                                ...
                                                {isEditing ? (
                                                    ...
                                                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                                                        ...
                                                        <ActBtn label="⏸ Disable" grad="linear-gradient(135deg,#f59e0b,#d97706)"
                                                            onClick={() => setConfirm({ text: 'You want to disable this community link!', fn: () => handleDisable(row.id) })} />
                                                        <ActBtn label="✕" grad="linear-gradient(135deg,#dc2626,#b91c1c)" small
                                                            onClick={() => setConfirm({ text: 'Once deleted, you will not be able to recover this community link!', fn: () => handleDelete(row.id) })} />
                                                    </div>
                                                )}
                                            </td>
                                        </tr>
