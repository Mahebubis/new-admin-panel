...
export default function CampaignWizard() {
    ...
    (async () => {
      ...
      try {
        ...
        if (res.data.success) {
          ...
          setDraft(d => ({
            ...d, id: c.id, name: c.name || '', tags: c.tags ? c.tags.split(',').map(t => t.trim()).filter(Boolean) : [],
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setDraft(d => ({ ...d, id: newId, reachable_count: res.data.data.reachable_count ?? d.reachable_count, status: res.data.data.status ?? d.status }));
        return newId;
      }
  ...
  const StepBody = [
    ...
    <CampaignStepSchedule key="schedule" draft={draft} setField={setField} onValidChange={v => setValid('schedule', v)} saveDraft={saveDraft} />,
  ][current];
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", height: '100vh', display: 'flex', flexDirection: 'column', background: '#f8fafc' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto', display: 'flex' }}>
        <div style={{ flex: 1, padding: 26 }}>{StepBody}</div>
        {current !== 2 && <CampaignSummaryPanel draft={draft} segmentNames={segmentNames} listNames={listNames} />}
      </div>
    </div>
  );
}
