...
export default function WaInbox() {
   ...
   * for eight hours would otherwise make 4,800 pointless requests — and resumed with an
   * immediate tick so switching back never shows stale messages.
   */
  useEffect(() => {
    let cancelled = false;
    let timer = null;
    ...
    const tick = async () => {
      ...
      try {
        ...
        // Silent: a dropped poll is not worth a toast every six seconds. The next tick recovers,
        // and anything the admin actually initiates still reports its own errors.
      }
    };
