...
function MultiPicker({ items, selected, onChange, placeholder, noun, showId = true, loading = false }) {
  const [open, setOpen] = useState(false);
  const [s, setS] = useState('');
  const ref = useOutsideClose(open, setOpen);
  const sel = selected || [];
        ...
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, width: 330, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 8, boxShadow: '0 10px 24px rgba(0,0,0,.15)', zIndex: 150 }}>
          ...
          <div style={{ maxHeight: 260, overflowY: 'auto' }}>
            {loading
              ...
              : filtered.length === 0
                ? <div style={{ padding: 14, fontSize: 12, color: '#94a3b8', textAlign: 'center' }}>{items.length ? 'No matches' : `No ${noun.toLowerCase()}s found`}</div>
                : filtered.map(it => {
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6 }}>
        ...
        {src.type === 'tags' ? (
          <MultiPicker items={tagItems} selected={src.tags} onChange={tags => setSrc({ tags })}
            placeholder={st.pick} noun={st.noun} showId={false} loading={loading} />
        ) : (
          <>
            ...
            {src.scope === 'specific' && (
              <MultiPicker items={items} selected={src.ids} onChange={ids => setSrc({ ids })}
                placeholder={st.pick} noun={st.noun} loading={loading} />
            )}
          </>
        )}
        <DayPicker day={cond.day} onChange={day => set({ day })} />
...
function Block({ block, onChange, onDelete, isFirst, blockNumber }) {
  ...
  return (
    <div style={{ position: 'relative', padding: 14, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, marginBottom: 10 }}>
      ...
      <button type="button" onClick={addCond} className="nc-add-anim" style={{ marginTop: 8 }}>
        <span>+ ADD</span>
      </button>
    </div>
  );
...
export default function NetcoreSegmentCreate() {
  ...
  const [count, setCount]             = useState(null);   // { total, email, phone } | null
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.post(API, new URLSearchParams({ action: 'engagement_options' }), FORM);
        if (res.data.status === 'success') {
          const d = res.data;
          setOpts({
            email_campaigns: d.email_campaigns || [], wa_campaigns: d.wa_campaigns || [], journeys: d.journeys || [],
            email_tags: d.email_tags || [], wa_tags: d.wa_tags || [], journey_tags: d.journey_tags || [],
            lists: d.lists || [], segments: d.segments || [],
          });
        } else toast.error(res.data.message || 'Failed to load campaigns and lists');
      } catch (err) { toast.error(errMsg(err, 'Failed to load campaigns and lists')); }
    })();
  }, []);
    ...
    (async () => {
      try {
        ...
        if (res.data.status === 'success') {
          ...
          setCount({ total: seg.user_count, email: seg.email_count, phone: seg.phone_count });
        }
      } catch (err) { toast.error(errMsg(err, 'Failed to load segment')); }
    })();
  ...
  const checkConfig = cfg => {
    ...
    if (!hasInclude) { toast.error('Add at least one condition under Include users'); return false; }
    const problem = validateSegmentConfig(cfg);
  ...
  const getCount = async () => {
    ...
    try {
      const res = await api.post(API, new URLSearchParams({ action: 'count', config: JSON.stringify(cfg) }), FORM);
      if (res.data.status === 'success') setCount({ total: res.data.count, email: res.data.email_count, phone: res.data.phone_count });
      else toast.error(res.data.message || 'Failed');
    } catch (err) { toast.error(errMsg(err, 'Network error')); } finally { setCounting(false); }
  };
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const params = new URLSearchParams({ action: 'save', name: name.trim(), contact_type: contactType, config: JSON.stringify(cfg) });
    ...
    } catch (err) { toast.error(errMsg(err, 'Network error')); } finally { setSaving(false); }
  };
        ...
        <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: 20 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 14, marginBottom: 14 }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #1e3a8a', borderRadius: 8, padding: '14px 22px', minWidth: 150 }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 700, color: '#0f172a' }}>{fmt(count?.total)}</div>
            </div>
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 8, padding: '14px 22px', display: 'flex', gap: 28 }}>
              <div>
                <div style={{ fontSize: 11.5, color: '#64748b', marginBottom: 4 }}>Reachable by Email</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>{fmt(count?.email)}</div>
              </div>
              <div>
                <div style={{ fontSize: 11.5, color: '#64748b', marginBottom: 4 }}>With phone (SMS / WhatsApp)</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>{fmt(count?.phone)}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.45)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div style={{ background: '#fff', borderRadius: 12, width: 'min(420px, 100%)', boxShadow: '0 20px 60px rgba(0,0,0,.3)', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '14px 18px', borderTop: '1px solid #e2e8f0', background: '#f8fafc' }}>
                ...
                <button type="button" onClick={() => { setExitOpen(false); nav('/netcore/segments'); }}
