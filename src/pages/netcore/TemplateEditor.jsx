...
export default function TemplateEditor() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (!isEdit) nav(`/netcore/templates/${res.data.data.id}`, { replace: true });
  ...
  const upload = async () => {
    ...
    try {
      ...
      if (res.data.success) { toast.success('Template uploaded'); nav(`/netcore/templates/${res.data.data.id}`, { replace: true }); }
  ...
  const TopBar = ({ onSave }) => (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 26px', borderBottom: '1px solid #e2e8f0', background: '#fff', flexShrink: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <button onClick={() => nav('/netcore/templates')} style={{ border: 'none', background: '#f1f5f9', borderRadius: 8, width: 34, height: 34, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: '#334155' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 26, maxWidth: 640, margin: '0 auto', width: '100%' }}>
          <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: 22, marginBottom: 18 }}>
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <input style={inp} value={category} onChange={e => setCategory(e.target.value)} placeholder="Choose category or add new" />
            </div>
          </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 26, maxWidth: 640, margin: '0 auto', width: '100%' }}>
          <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: 22 }}>
            ...
            <div style={{ border: '2px dashed #cbd5e1', borderRadius: 10, padding: 30, textAlign: 'center' }}>
              <input type="file" accept=".htm,.html" onChange={e => setFile(e.target.files?.[0] || null)} style={{ fontSize: 12.5 }} />
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden' }}>
        <div style={{ width: 340, flexShrink: 0, borderRight: '1px solid #e2e8f0', background: '#fff', padding: 20, overflowY: 'auto' }}>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <input style={inp} value={subject} onChange={e => setSubject(e.target.value)} placeholder="Used to prefill a campaign's subject" />
          </div>
        </div>
