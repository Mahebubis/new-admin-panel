...
import AttributeLogs from './AttributeLogs';
...
export default function NetcoreAttributes() {
  ...
  const fetchPage = async (p = page, pp = perPage, s = appliedSearch) => {
    ...
    try {
      const res = await api.post(API, new URLSearchParams({ action: 'list', page: p, per_page: pp, search: s }), FORM);
      if (res.data.success) {
        setRows(res.data.data.attributes || []);
  ...
  const doDelete = async (id) => {
    setMenuFor(null);
    if (!window.confirm('Delete this attribute? Any campaign template still using its [NAME] token will show a blank value instead.')) return;
  ...
  return (
    <>
      <style>{`
        ...
        .nc-attr *{ box-sizing:border-box; font-family:'Plus Jakarta Sans',sans-serif; }
        .nc-attr-row .nc-attr-dots { opacity: 1; background: none; border: none; cursor: pointer; padding: 4px; color: #1e293b; transition: color .15s; }
        .nc-attr-row .nc-attr-dots:hover { color: #1e3a8a; }
        .nc-attr-dots.menu-open { color: #1e3a8a; }
        .nc-attr-row:hover td { background: #f5f3ff; }
      `}</style>

      <div className="nc-attr" style={{ padding: 24, height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {rows.length === 0 && !loading
                  ? <tr><td colSpan={6} style={{ padding: 32, textAlign: 'center', color: '#94a3b8' }}>{appliedSearch ? `No attributes matching "${appliedSearch}".` : 'No attributes yet. Click "Create New" to add one.'}</td></tr>
                  : rows.map(r => (
                      <tr key={r.id} className="nc-attr-row">
                        ...
                        <td style={{ padding: '14px 18px', borderBottom: '1px solid #f1f5f9' }}>
                          <div style={{ position: 'relative' }}>
                            <button className={`nc-attr-dots${menuFor === r.id ? ' menu-open' : ''}`}
                            ...
                            {menuFor === r.id && (
                              <div ref={menuRef}
                                style={{ position: 'absolute', top: 'calc(100% + 4px)', right: 0, background: '#fff', borderRadius: 8, boxShadow: '0 8px 24px rgba(0,0,0,.12)', border: '1px solid #e2e8f0', zIndex: 50, width: 160, padding: 6 }}>
      ...
      {logsOpen && <AttributeLogs onClose={() => setLogsOpen(false)} />}
    </>
  );
}
