/*
 * Kumo — shared look-and-feel for the attribute create/edit drawer and the lookup-chain
 * builder it embeds.
 *
 * A copy of src/pages/netcore/attributeUi.jsx so nothing in this module imports from the
 * netcore folder. The styles and the SearchDropdown behave exactly as they do there; the
 * only things left behind are the netcore API/FORM constants, because everything in this
 * module talks to api/kumo/kumo.php through kapi() instead.
 */
...
/** An input that is disabled in the drawer's read-only mode, greyed out the same way a
 *  locked mapped field is. */
export const roInp = { ...inp, background: '#f8fafc', color: '#64748b', cursor: 'not-allowed' };
 ...
 * onSearch(term) (debounced) and lists whatever comes back. Kumo has no column/table search
 * endpoint, so nothing in this module passes onSearch today — it is kept so the component
 * stays a drop-in copy of the Netcore one.
 */
