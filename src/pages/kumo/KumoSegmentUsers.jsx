/*
 * KumoMTA — the people inside one segment.
 *
 * A clone of the Netcore segment members screen (NetcoreSegmentUsers): the
 * same breadcrumb, the same single-tab toolbar with refresh / search / CSV /
 * column chooser, the same sticky table and the same per-page footer.
 *
 * Data layer only: kapi('segment_users', { id, page, per_page }) →
 * { rows, total, page, per_page }. The Kumo member row carries email,
 * first_name and last_name, so those are the columns.
 */
...
function Spinner({ size = 32 }) {
  return <span style={{ display: 'inline-block', width: size, height: size, borderRadius: '50%', border: '3px solid #c4b5fd', borderTopColor: '#4f46e5', animation: 'km_su_spin 0.85s linear infinite' }} />;
}
...
export default function KumoSegmentUsers() {
  const { id } = useParams();
  ...
  const fetchPage = async (p = 1, pp = perPage) => {
    setLoading(true);
    try {
      const d = await kapi('segment_users', { id, page: p, per_page: pp });
      setRows(d.rows || []);
  ...
  useEffect(() => { fetchPage(1, perPage); }, [id]); // eslint-disable-line react-hooks/exhaustive-deps

  /* the segment's name — segments_list carries it, there is no per-segment read */
  useEffect(() => {
    let alive = true;
    kapi('segments_list')
      .then(d => { if (!alive) return; const s = (d.segments || []).find(x => String(x.id) === String(id)); if (s) setName(s.name || ''); })
      .catch(() => {});
    return () => { alive = false; };
  }, [id]);
  ...
  const onSearchKey = e => {
    if (e.key === 'Enter') setApplied(searchVal);
    if (e.key === 'Escape') { setSearchOpen(false); if (appliedSearch) { setSearchVal(''); setApplied(''); } }
  };
  const toggleSearch = () => {
    setSearchOpen(o => {
      const next = !o;
      if (next) setTimeout(() => searchInputRef.current?.focus(), 50);
      else if (appliedSearch) { setSearchVal(''); setApplied(''); }
      return next;
    });
  };

  /* the API pages server-side, so the search narrows the page you are looking at */
  const visibleRows = useMemo(() => {
    const q = appliedSearch.trim().toLowerCase();
    if (!q) return rows;
    return rows.filter(r => `${r.email || ''} ${r.first_name || ''} ${r.last_name || ''}`.toLowerCase().includes(q));
  }, [rows, appliedSearch]);

  const shownCols = ALL_COLS.filter(c => colsVisible[c.key]);

  const downloadCsv = () => {
    if (!visibleRows.length) { toast.error('Nothing to download'); return; }
    const esc = v => `"${String(v ?? '').replace(/"/g, '""')}"`;
    const csv = [shownCols.map(c => esc(c.label)).join(','),
      ...visibleRows.map(r => shownCols.map(c => esc(r[c.key])).join(','))].join('\r\n');
    const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8;' }));
    const a = document.createElement('a');
    a.href = url; a.download = `kumo_segment_${id}_page_${page}.csv`;
  ...
  return (
    <>
      <style>{`
        @keyframes km_su_spin { to { transform: rotate(360deg); } }
        .km-su *{ box-sizing:border-box; font-family:'Plus Jakarta Sans',sans-serif; }
        .km-su-row:hover td { background: #f5f3ff; }
        .km-su-truncate { max-width: 220px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      `}</style>

      <div className="km-su" style={{ padding: 20, height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

        {/* breadcrumb / title */}
        <div style={{ flexShrink: 0, marginBottom: 14 }}>
          <Link to="/kumo/segments"
          ...
          <h2 style={{ margin: '6px 0 0', fontSize: 18, fontWeight: 700, color: '#0f172a' }}>
            {name} <span style={{ color: '#64748b', fontWeight: 600 }}>(Contact Count: {total.toLocaleString()})</span>
          </h2>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 10, padding: '14px 18px', marginBottom: 12, boxShadow: '0 1px 3px rgba(0,0,0,.05)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #e2e8f0' }}>
            <div style={{ padding: '6px 4px', borderBottom: '2px solid #1e3a8a', color: '#1e3a8a', fontSize: 13, fontWeight: 600, marginBottom: -1 }}>
              Members <span style={{ color: '#94a3b8', fontWeight: 500, marginLeft: 4 }}>({total.toLocaleString()})</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              <IconBtn title="Refresh" onClick={() => fetchPage(page, perPage)}>
              ...
              {searchOpen ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, border: '1.5px solid #1e3a8a', borderRadius: 8, background: '#fff', padding: '0 8px 0 10px', height: 34 }}>
                  ...
                  <input ref={searchInputRef} value={searchVal} onChange={e => setSearchVal(e.target.value)} onKeyDown={onSearchKey} placeholder="Filter this page and press Enter…"
              ...
              <IconBtn title="Download this page as CSV" onClick={downloadCsv}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>
                {visibleRows.length === 0 && !loading
                  ? <tr><td colSpan={Math.max(1, shownCols.length)} style={{ padding: 32, textAlign: 'center', color: '#94a3b8' }}>
                      {appliedSearch ? `No members on this page matching "${appliedSearch}".` : 'No members'}
                    </td></tr>
                  : visibleRows.map((r, i) => (
                    <tr key={`${r.email}-${i}`} className="km-su-row">
                      {shownCols.map((c, ci) => (
                        <td key={c.key} style={{ padding: '11px 14px', borderBottom: '1px solid #f1f5f9', color: ci === 0 ? '#1e3a8a' : '#334155', fontWeight: ci === 0 ? 600 : 400 }}>
                          <div className="km-su-truncate" title={r[c.key] || ''}>{r[c.key] || 'NA'}</div>
                        </td>
                      ))}
                    </tr>
                  ))
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12, fontSize: 13, color: '#475569', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span>Per page:</span>
            <select value={perPage} onChange={e => { const n = parseInt(e.target.value, 10); setPerPage(n); fetchPage(1, n); }}
