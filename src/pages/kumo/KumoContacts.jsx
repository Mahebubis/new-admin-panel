 ...
 * Kumo — All contacts.
 *
 * A clone of the Netcore "All contacts" screen so the two modules show the same
 * people in the same shape: these rows are the panel's real users, read from the
 * same read-only endpoint the Netcore Audience screen uses.
 *
 * The only difference from the Netcore original is where an email takes you —
 * /kumo/contacts/:email instead of /netcore/contacts/:email.
 */
...
function Spinner({ size = 32 }) {
  return (
    <span style={{
      ...
      animation: 'km_c_spin 0.85s linear infinite'
    }} />
  );
}
...
export default function KumoContacts() {
  ...
  /* The click-through habit from the Netcore screen, pointed at the Kumo route.
     Kept as a real href so middle-click and "open in new tab" still work. */
  const contactHref = (r) => {
    const base = `/kumo/contacts/${encodeURIComponent(r.email || '')}`;
    return r.user_id ? `${base}?uid=${encodeURIComponent(r.user_id)}` : base;
  };
  const openContact = (e, r) => {
    if (e.metaKey || e.ctrlKey || e.shiftKey || e.button === 1) return; /* let the browser have it */
    e.preventDefault();
    navigate(contactHref(r));
  };

  return (
    <>
      <style>{`
        @keyframes km_c_spin { to { transform: rotate(360deg); } }
        .km-c-root *{box-sizing:border-box;font-family:'Plus Jakarta Sans',sans-serif;}
        .km-c-row:hover td { background: #f5f3ff; }
        .km-c-email { color: #2563eb; cursor: pointer; text-decoration: none; }
        .km-c-email:hover { text-decoration: underline; }
        .km-c-truncate { max-width: 220px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      `}</style>

      <div className="km-c-root" style={{ padding: 20, height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>
                {rows.length === 0 && !loading
                  ...
                  : rows.map((r, i) => (
                    <tr key={`${r.user_id}-${i}`} className="km-c-row">
                      <td style={{ padding: '11px 14px', borderBottom: '1px solid #f1f5f9' }}>
                        <a href={contactHref(r)} onClick={e => openContact(e, r)} className="km-c-email">
                          {r.email}
                        </a>
                      </td>
                      {COLS.slice(1).map(c => (
                        <td key={c.key} style={{ padding: '11px 14px', borderBottom: '1px solid #f1f5f9', color: '#334155' }}>
                          <div className="km-c-truncate" title={r[c.key] || ''}>{r[c.key]}</div>
                        </td>
                      ))}
                    </tr>
                  ))
