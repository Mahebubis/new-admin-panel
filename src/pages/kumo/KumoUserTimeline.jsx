 ...
 * Kumo — user timeline.
 *
 * A clone of the Netcore user timeline, reading the same read-only endpoint so
 * both modules describe the same person the same way. Two differences:
 *
 *   1. it lives at /kumo/contacts/:email and goes back to /kumo/contacts;
 *   2. it opens with a Kumo-specific strip — is this address suppressed on the
 *      Kumo blocklist, and what has the Kumo MTA actually sent them.
 */
import { useEffect, useRef, useState } from 'react';
import { useParams, useSearchParams, useLocation, useNavigate } from 'react-router-dom';
import api from '../../api/axios';
import { KumoStyles, kapi, Pill, nf, ago, fmtDt as kumoFmtDt } from './kumoShared';
      ...
      {Array.from({ length: dots }).map((_, i) => {
        ...
        return (
          <span key={i} style={{
            ...
            background: '#1e3a8a',
            animation: 'km_tl_dot 1.2s linear infinite',
...
const quietLine = { fontSize: 12.5, color: '#94a3b8', padding: '2px 0' };

/*
  What the Kumo platform itself knows about this address: whether it is suppressed, and the last
  handful of messages it actually pushed out. Deliberately plain — the same white card as the rest
  of this page, not the frosted glass of the Kumo dashboards, because it is a strip on somebody
  else's screen rather than a card of its own.
*/
function KumoSendingStrip({ email }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true); setFailed(false); setBlock(null); setSends([]); setTotalSends(0);

    Promise.allSettled([
      kapi('blocklist_list', { search: email, per_page: 1 }),
      kapi('message_log',    { search: email, per_page: 20 }),
    ]).then(([bl, ml]) => {
      if (cancelled) return;
      let anyOk = false;
      ...
      if (bl.status === 'fulfilled') {
        anyOk = true;
        const rows = bl.value?.rows || [];
        /* the search is a LIKE, so only an exact address counts as "this person is blocked" */
        const hit = rows.find(r => String(r.email || '').toLowerCase() === String(email).toLowerCase());
        setBlock(hit || null);
      }
      if (ml.status === 'fulfilled') {
        anyOk = true;
        const rows = ml.value?.rows || [];
        setSends(rows.filter(r => !r.email || String(r.email).toLowerCase() === String(email).toLowerCase()));
        setTotalSends(Number(ml.value?.total || 0));
      }
      if (!anyOk) setFailed(true);
  ...
  const shell = (children) => (
    <div style={{
      flexShrink: 0, background: '#fff', borderRadius: 10, padding: '12px 18px',
      ...
      <div style={{
        ...
        <span style={{ fontWeight: 700, fontSize: 13, color: '#0f172a' }}>Kumo MTA sending</span>
        {!loading && !failed && totalSends > 0 && (
          <span style={{ fontSize: 11.5, color: '#64748b', fontWeight: 600 }}>
            {nf(totalSends)} {totalSends === 1 ? 'message' : 'messages'} on record
          </span>
        )}
        {!loading && block && (
          <Pill tone="red">
            Blocklisted · {BLOCK_REASON_LABEL[block.reason] || block.reason || 'suppressed'}
          </Pill>
        )}
      </div>
      {children}
  ...
  if (loading) return shell(<div style={quietLine}>Checking the Kumo platform…</div>);
  if (failed)  return shell(<div style={quietLine}>Could not reach the Kumo platform just now.</div>);

  if (!block && sends.length === 0) {
    return shell(<div style={quietLine}>No sends from the Kumo platform yet</div>);
  }

  return shell(
    <>
      {block && (
        <div style={{ fontSize: 12, color: '#b42318', marginBottom: sends.length ? 10 : 0, lineHeight: 1.6 }}>
          Suppressed{block.created_at ? ` on ${kumoFmtDt(block.created_at)}` : ''}
          {block.detail ? ` — ${block.detail}` : ''}. Campaigns skip this address before they queue.
        </div>
      )}

      {sends.length === 0 ? (
        <div style={quietLine}>No sends from the Kumo platform yet</div>
      ) : (
        <div style={{ maxHeight: 156, overflowY: 'auto', margin: '0 -4px' }}>
          {sends.map((s, i) => {
            const st = String(s.status || '').toLowerCase();
            const opens = Number(s.open_count || 0);
            const clicks = Number(s.click_count || 0);
            const when = s.sent_at || s.delivered_at || s.opened_at || s.clicked_at;
            return (
              <div key={s.id ?? `${when}-${i}`}
                style={{
                  display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 120px 110px 92px 108px',
                  alignItems: 'center', gap: 10, padding: '7px 4px',
                  borderTop: i === 0 ? 'none' : '1px solid #f1f5f9', fontSize: 12,
                }}>
                <div title={s.campaign || ''}
                  style={{ color: '#0f172a', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {s.campaign || 'Untitled campaign'}
                  {s.error ? <span style={{ color: '#b42318', fontWeight: 500 }}> · {s.error}</span> : null}
                </div>
                <div title={s.provider || ''} style={{ color: '#64748b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {s.ip_addr || '—'}
                </div>
                <div>
                  <Pill tone={KUMO_STATUS_TONE[st] || 'slate'}>
                    {s.status || 'unknown'}{s.smtp_code ? ` ${s.smtp_code}` : ''}
                  </Pill>
                </div>
                <div style={{ color: opens || clicks ? '#0e7490' : '#94a3b8', fontWeight: 600, whiteSpace: 'nowrap' }}>
                  {nf(opens)} op · {nf(clicks)} cl
                </div>
                <div title={when ? kumoFmtDt(when) : ''} style={{ color: '#64748b', whiteSpace: 'nowrap', textAlign: 'right' }}>
                  {when ? ago(when) : '—'}
                </div>
              </div>
            );
...
export default function KumoUserTimeline() {
  ...
  const backTo    = location.state?.backTo    || '/kumo/contacts';
  ...
  */
  ...
  return (
    <>
      <KumoStyles />
      <style>{`
        @keyframes km_tl_dot { 0% { opacity: .15; } 25% { opacity: 1; } 100% { opacity: .15; } }
        @keyframes km_tl_fadein { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: none; } }
        .km-tl-page *{ box-sizing:border-box; font-family:'Plus Jakarta Sans',sans-serif; }
        .km-tl-row { animation: km_tl_fadein .25s ease both; transition: background .15s; }
        .km-tl-row:hover { background: #f5f3ff !important; cursor: pointer; }
        .km-tl-event-link { color: #1e3a8a; font-weight: 600; font-size: 13px; text-decoration: none; }
        .km-tl-event-link:hover { text-decoration: underline; }
        .km-tl-desc { color: #94a3b8; font-size: 11.5px; margin-top: 3px; }
        .km-tl-desc b { color: #475569; font-weight: 600; }
        .km-tl-back { color: #1e3a8a; text-decoration: none; }
        .km-tl-back:hover { text-decoration: underline; }
      `}</style>

      <div className="km-tl-page" style={{
        ...
        <div style={{ flexShrink: 0 }}>
          <a href={backTo} className="km-tl-back"
            onClick={e => {
        ...
        {/* what the Kumo platform itself has done with this address */}
        <KumoSendingStrip email={email || profile?.email} />
              ...
              <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }} onScroll={onListScroll}>
                {filteredEvents.length === 0
                  ...
                  : visibleEvents.map((e, i) => {
                    ...
                    return (
                      <div key={`${e.event}-${e.timestamp}-${i}`} className="km-tl-row"
                        onClick={() => openEvent(e)}
                        style={{
                          display: 'grid', gridTemplateColumns: '1fr 100px 80px 180px',
